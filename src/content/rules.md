...
- **Rule #1:** We play fair. No attempts to gain root access, no DOS, no attacks on the infrastructure or our fellow students and instructors. We extract flags and submit them - we do **_NOT_** `rm -rf /` and so on. We DO NOT take actions that prevent our classmates from getting SLA points or communicating with the infrastructure. Failure to do so will result in a grade of zero, but even worse, it'll ruin the experience for everyone.
...
**Important**: in each window, you have a quota of 1,000 flag submissions per api_key. *DO* not exceed 1,000 submissions or your api_key may be throttled. If - by chance - you happen to gain access to another team's api_key it is *forbidden* to use this key to exhaust another team's quota (Rule #1).
